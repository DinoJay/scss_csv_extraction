export const paragraphQuery = (variable: string | string[] | undefined, paragraphText, category = "") => {
	...
	// 	`.trim();
	return `
	Find the value for the following variable "${variable}" based on the category ${category} in the following text namely "${paragraphText}". If you don't know the answer, respond with "-". Don't include any commentary text or command strings such as "\`\`\`"! 
	`.trim();
}
