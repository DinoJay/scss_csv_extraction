export const paragraphQuery = (variable: string | string[] | undefined, paragraphText, category = undefined) => {
	...
	// 	`.trim();
	return `
	Find the value for the following variable "${variable}" ${category ? `based on the category "${category}"` : ''} in the following text namely "${paragraphText}". If you don't know the answer, respond with "-". Don't include any commentary text or command strings such as "\`\`\`"! 
	`.trim();
}
